...
std::vector<std::string> getTransformCombinations(std::array<std::string, 4>, int);
...
int main(int argc, const char * argv[])
{
    ...
    if(argc == 2 && string(argv[1]) == string("-h")){
        cout << "Usage: " << argv[0] << "[benchmarkNr] ["<< positiveSentiments <<"] [" << negativeSentiments << "] [" << shufPath << "]" << endl;
        cout << "       1. BenchmakrNr: 0 for new, (TIMESTAMP) for resuming..." << endl;
        cout << "       2. Argument: Path to positive Sentiments File" << endl;
        cout << "       3. Argument: Path to negative Sentiments File" << endl;
        cout << "       4. Argument: Path to (g)shuf binary" << endl;
        exit(0);
    }
    ...
    if(argc >= 2){
        benchmarkNr = string(argv[1]);
    }
    if(argc >= 3){
        positiveSentiments = string(argv[2]);
    }
    if(argc >= 4){
        negativeSentiments = string(argv[3]);
    }
    if(argc >= 5){
        shufPath = string(argv[4]);
    }
